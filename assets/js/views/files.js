...
export async function renderDashboard() { const data = await api(`/api/dashboard?${state.folderId ? `folderId=${state.folderId}` : ''}`); state.dashboard = data; state.folderId = data.folderId || null;
  ...
  const crumb = `<span class="crumb" data-folder="">MyDrive</span>${(data.path || []).map((entry, index, list) => `<span>/</span><span class="crumb${index === list.length - 1 ? ' current' : ''}" data-folder="${entry.id}">${esc(entry.name)}</span>`).join('')}`;
